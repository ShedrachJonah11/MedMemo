...
export const verifyGoogle = async (code:any,code_verifier:any,state:any)=>{
let req= await axios.get(`${APP_URL}/auth/google/callback?code=${code}&state=${state}`);
return req.data;
}
...
let req= await AxiosInstance.get(`/api/manage/subscription?sub_type=WEB&action=${action}`);
return req.data;
}
